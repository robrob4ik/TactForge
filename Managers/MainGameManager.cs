// Assets/PROJECT/Scripts/Runtime/Game/MainGameManager.cs
using UnityEngine;
...
namespace OneBitRob
{
    ...
    public sealed class MainGameManager : MonoBehaviour
    {
        ...
        [Header("Debug / Tools")]
        public SpellDebugConfig spellDebugConfig;
        ...
        private void Awake()
        {
            ...
            // 3) Spell debug config (optional, but we prefer it centralized)
            GameServices.SpellDebug = spellDebugConfig;

            // 4) Scene services (resolve or create fallbacks)
            GameServices.ProjectilePools = projectilePools ? projectilePools : FindObjectOfType<ProjectilePoolManager>(true);
            GameServices.SpellVfx       = spellVfx       ? spellVfx       : FindObjectOfType<SpellVfxPoolManager>(true);
        }
    }
}
